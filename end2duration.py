...
def handle_file_processing_and_reporting(file_processor: FileProcessor):
    """
    处理用户输入的文件或目录路径，进行内容校验，报告结果，并计时。
    :param file_processor: FileProcessor的实例。
    :return: False 表示程序应退出，True 表示可以继续下一轮输入。
    """
    try:
        # 获取用户输入
        user_input_str = input("请输入文本文件路径或目录路径 (或输入 'q' 退出): ")
        if user_input_str.lower() == 'q': # 用户选择退出
            return False

        if not user_input_str: # 用户未输入任何内容
            print("未输入路径。请重新输入。")
            return True # 继续循环
        ...
        if os.path.isdir(user_input_str):
            ...
            print(f"\n{Colors.GREEN}正在递归扫描目录 '{user_input_str}' 及其所有子目录...{Colors.RESET}")
            # 遍历目录及其所有子目录
        ...
        # 如果输入的是文件
            ...
            if user_input_str.lower().endswith(".txt"): # 检查是否是 .txt 文件
        ...
        files_processed_count = 0 # 已处理文件计数
        ...
        for i, current_file_path in enumerate(files_to_process):
            ...
            # 打印当前处理的文件路径
            # simplified_path 用于尝试显示相对路径，但直接显示完整路径更简单明了
        ...
        elif not is_directory_scan and files_processed_count == 1 : # 如果是单个文件处理
            if files_with_errors_count_in_batch > 0:
                print(f"{Colors.RED}该文件存在错误。{Colors.RESET}")
    
    except EOFError: # 捕获 Ctrl+D (Unix) 或 Ctrl+Z+Enter (Windows) 导致的 EOFError
        print("\n检测到EOF，程序即将退出。")
        return False # 程序退出
...
def main():
    ...
    while True:
        ...
        if not should_continue: # 如果 handle_file_processing_and_reporting 返回 False，则退出循环
            break
    print("程序已退出。")
